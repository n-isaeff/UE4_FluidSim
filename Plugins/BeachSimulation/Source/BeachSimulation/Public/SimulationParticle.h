...
struct BEACHSIMULATION_API FParticle
{
	...
	FParticle(const FVector& pos, float m);
...
struct BEACHSIMULATION_API FExternalForce
{
	...
	virtual void ApplyForce(FParticle* p);
};
...
struct BEACHSIMULATION_API FGravityForce : public FExternalForce
{
	...
	virtual void ApplyForce(FParticle* p) override;
};

//struct BEACHSIMULATION_API FPlaneProximityForce : public FExternalForce
//{
//	FVector Normal;
//	float Offset, Strength, Radius;
//
//	inline FPlaneProximityForce(const FVector& norm, float offset, float strength, float r)
//		: Normal{norm}
//		, Offset{offset}
//		, Strength{ strength }
//		, Radius{ r }
...
struct BEACHSIMULATION_API FDomainGrid
{
	...
	void ForNeighborParticles(FParticle* p, TFunction<void(FParticle*, FParticle*, float)> body);
